...
// =========================================================
// Main SidebarApp Component - NOW FULLY ROUTE-BASED
// =========================================================
...
const OwnerSidebar = ({ isCollapsed, setIsCollapsed }: SidebarProps) => {
  ...
  const handleTabClick = (path: string, text: string) => {
    ...
    // Navigate only if not already on that page
  ...
  const buttonBaseClass = `lg:hidden fixed top-3 p-2 z-50 bg-white border rounded shadow transition-all duration-300`;

  return (
    <div className="flex bg-white">
      <Topbar
        sidebarCollapsed={isCollapsed}
        onProfileClick={() => handleTabClick("/dashboard-owner/profile", "My Profile")}
      />
          ...
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex gap-3 mt-5">
              <button
                ...
                className={`flex-1 py-2.5 px-2 rounded-lg font-medium transition-all flex items-center justify-center gap-2 ${isLoggingOut
